...
class ParserVariableRequests:

    def __init__(self, simConnect):
        logging.info("ParserVariableRequests __init__")
        self.sm = simConnect
        self.sim_vars = {}
    ...
    def subscribe_to_data_change(self, data_area_id, request_id, definition_id):
        logging.info("subscribe_to_data_change data_area_id=%s, request_id=%s, definition_id=%s", data_area_id,
                     request_id, definition_id)
    ...
    def send_data(self, data_area_id, definition_id, size, dataBytes):
        logging.info("send_data data_area_id=%s, definition_id=%s, size=%s, dataBytes=%s", data_area_id, definition_id,
                     size, dataBytes)
    ...
    def send_command(self, command):
        ...
        data_byte_array.extend(
            bytearray(self.DATA_STRING_SIZE - len(data_byte_array)))  # extend to fix DATA_STRING_SIZE
        self.send_data(self.CLIENT_DATA_AREA_CMD, self.DATA_STRING_DEFINITION_ID, self.DATA_STRING_SIZE,
    ...
    def initialize_client_data_areas(self):
        ...
        self.sm.dll.MapClientDataNameToID(self.sm.hSimConnect, "Parser.LVars".encode("ascii"),
        ...
        self.sm.dll.MapClientDataNameToID(self.sm.hSimConnect, "Parser.Command".encode("ascii"),
                                          self.CLIENT_DATA_AREA_CMD)
        self.sm.dll.CreateClientData(self.sm.hSimConnect, self.CLIENT_DATA_AREA_CMD, self.DATA_STRING_SIZE,
                                     self.FLAG_DEFAULT)
        # register client data area for receiving responses
        self.sm.dll.MapClientDataNameToID(self.sm.hSimConnect, "Parser.Response".encode("ascii"),
                                          self.CLIENT_DATA_AREA_RESPONSE)
        self.sm.dll.CreateClientData(self.sm.hSimConnect, self.CLIENT_DATA_AREA_RESPONSE, self.DATA_STRING_SIZE,
                                     self.FLAG_DEFAULT)
        # subscribe to WASM Module responses
        self.add_to_client_data_definition(self.DATA_STRING_DEFINITION_ID, self.DATA_STRING_OFFSET,
                                           self.DATA_STRING_SIZE)
        self.subscribe_to_data_change(self.CLIENT_DATA_AREA_RESPONSE, self.DATA_STRING_DEFINITION_ID,
