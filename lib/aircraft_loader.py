...
class AircraftLoader:
    ...
    def set_initial_values(self, aircraft_array: int, cpflight_cmds:dict, sock, vr):
        ...
        self.led_fo_efis = {k: not bool(vr.get(f"({v.get('rx')})")) for k, v in aircraft_array.items() if
                        k in self.led_fo_efis and v.get("rx")}
    ...
    def set_led_efis_cp(self, aircraft_array: int, cpflight_cmds:dict, sock, vr) -> bool:
        logger.critical("Starting -> set_led_efis_cp")
        for key in self.led_cp_efis.keys():
            logger.critical(f"CP Key {key}")
            rx_expr = aircraft_array.get(key, {}).get("rx")
            logger.critical(f"CP rx_expr {rx_expr}")
            if rx_expr is None:
                logger.critical(f"CP CONTINUE")
                continue
            value = bool(vr.get(f"({rx_expr})"))
            logger.critical(f"CP value {value} against -> {self.led_cp_efis[key]}")
            if self.led_cp_efis[key] != value:
                self.led_cp_efis[key] = value
                cmd = cpflight_cmds.get(key, {}).get("led_on" if value else "led_off")
                if cmd:
                    logger.critical(f"Set LED {key} CMD CpFlight: {cmd}")
                    sock.sendall((cmd + "\n").encode())
        return True
    ...
    def set_btn_cp_fd_aircraft(self, value: str, config, vr, sock, cpfligh):
        ...
        logger.critical(f"set_btn_cp_fd_aircraft: {new_value}")
    ...
    def set_btn_cp_cstr_aircraft(self, value: str, config, vr, sock, cpfligh):
        ...
        for el in config['btn_cp_cstr']['tx']:
            actual = vr.get(f"({el})")
            vr.set(f"{int(actual+2)} (>{el})")
        sock.sendall((self._reset_leds_command_cp(which_led, new_value, cpfligh)).encode())
        self.btn_gen["op"] = False
    ...
    def set_btn_fo_cstr_aircraft(self, value: str, config, vr, sock, fofligh):
        ...
        for el in config['btn_fo_cstr']['tx']:
            actual = vr.get(f"({el})")
            vr.set(f"{int(actual+2)} (>{el})")
        sock.sendall((self._reset_leds_command_fo(which_led, new_value, fofligh)).encode())
        self.btn_gen["op"] = False
